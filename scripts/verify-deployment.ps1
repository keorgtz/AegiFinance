...
Require-Text "docker-compose.yml" 'Automation__PollIntervalSeconds:\s*\$\{AUTOMATION_POLL_INTERVAL_SECONDS:-30\}' "Outbox automation polling is not reproducibly configured."
Require-Text "docker-compose.yml" '(?s)backup:.*BACKUP DATABASE.*CHECKSUM.*RESTORE VERIFYONLY.*aegifinance-sql-backups' "Verified SQL backups are not encoded in Compose."
Require-Text "docker-compose.yml" 'aegifinance-worker-heartbeat' "The worker has no operational health contract."
...
Require-Text "src/AegiFinance.Web/Program.cs" '(?s)/health/live.*?/health/ready.*Database.CanConnectAsync' "API liveness and database readiness are not separated."
...
Require-Text "scripts/verify-backup-restore.sh" '(?s)RESTORE DATABASE.*DBCC CHECKDB.*evidence/phase15-restore.txt' "The deployment has no executable restore rehearsal."
