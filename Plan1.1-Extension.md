...
Este documento define reglas de arquitectura de usuarios, permisos y SVA — son independientes de la tecnología de UI.

Los ejemplos de código de este documento se muestran en **React + TypeScript** (rutas de Next.js y componentes `.tsx`), conforme al pivote de frontend definido en `Plan1.2-Extension.md`. Las entidades de dominio (`User`, `ClientUser`, `ClientPinCredential`, `Permission`, `SubscriptionPermission`) se implementan en C# dentro de `AegiFinance.Domain`, sin cambios.
...
`can()` proviene del hook `usePermissions()`, que lee los permisos efectivos del usuario autenticado desde `GET /api/auth/me`. Ver `Plan1.2-Extension.md`.
