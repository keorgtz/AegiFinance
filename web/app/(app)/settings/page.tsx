...
function CatalogManager({ kind, title, description, items, loading, permission, withColor, onCreate, onUpdate, onDelete }: {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!name.trim()) return; setSaving(true); try { const value = withColor ? { name: name.trim(), color: detail } : { name: name.trim(), description: detail.trim() || undefined }; if (editing) await onUpdate(editing.id, value); else await onCreate(value); close(); } finally { setSaving(false); } };
  ...
  return <section aria-labelledby={`${kind}-title`}>
    ...
    {formOpen && <form onSubmit={submit} className="mb-5 rounded-card border border-action/30 bg-action-soft p-4">
      <div className="grid gap-4 sm:grid-cols-[1fr_1.5fr_auto] sm:items-end">
        <Input controlKey={`settings.${kind}.field.name`} permission={permission} label="Nombre" value={name} onChange={(e) => setName(e.target.value)} maxLength={200} required autoFocus />
    ...
    {loading ? <div className="h-32 animate-pulse rounded-card bg-surface-subtle" /> : items.length === 0 ? <Empty text="Todavía no hay registros. Agregá el primero para habilitarlo en los formularios." /> : <div className="grid gap-3 md:grid-cols-2">
      {items.map((item) => <article key={item.id} className="flex min-h-20 items-center gap-3 rounded-card border border-border bg-surface p-4 shadow-dp1">
        ...
        <Button controlKey={`settings.${kind}.delete`} permission={permission} variant="danger" size="icon" onClick={() => confirm(`¿Eliminar "${item.name}"?`) && onDelete(item.id)} aria-label={`Eliminar ${item.name}`}><Trash2 className="h-4 w-4" /></Button>
      </article>)}
    </div>}
  </section>;
}
...
function CurrenciesPanel() {
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); if (editing) await update.mutateAsync({ id: editing.id, data: { name: form.name, symbol: form.symbol, isActive: form.isActive, isDefault: form.isDefault } }); else await create.mutateAsync({ ...form, code: form.code.toUpperCase() }); setOpen(false); };
...
function ExchangeRatesPanel() {
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); const payload = { ...form, effectiveDate: new Date(`${form.effectiveDate}T00:00:00Z`).toISOString() }; if (editing) await update.mutateAsync({ id: editing.id, data: { rateToMXN: payload.rateToMXN, rateFromMXN: payload.rateFromMXN, effectiveDate: payload.effectiveDate } }); else await create.mutateAsync(payload); setOpen(false); };
