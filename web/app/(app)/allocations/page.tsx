...
export default function PaymentApplicationsPage() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><h1 className="font-display text-[22px] font-bold text-foreground">Aplicación de pagos</h1><p className="mt-1 text-sm text-muted">Elige un cliente y distribuye sus pagos entre una o varias mensualidades. Puedes deshacer una asignación desde su recibo.</p></div>
...
function ManualDialog({ open, onOpenChange, clientId }: { open: boolean; onOpenChange: (open: boolean) => void; clientId: string }) {
  ...
  const subscriptions = [...new Set(charges.data?.items.map(item => item.subscriptionCode) ?? [])];
  ...
  const chargeOptions = charges.data?.items.filter((item) => (item.status === "Pending" || item.status === "Partial") && ((!period || item.periodStart.slice(0, 7) === period) && (!subscription || item.subscriptionCode === subscription) || lines.some(line => line.billingItemId === item.id))).map((item) => ({ value: item.id, label: item.description, description: `${item.subscriptionCode} · ${formatDate(item.periodStart)} — ${formatDate(item.periodEnd)} · saldo ${formatAmount(item.balance, item.currency)}` })) ?? [];
  ...
  return <Dialog open={open} onOpenChange={close}><DialogContent title="Aplicación manual" description="Distribuí uno o varios pagos entre cargos. El servidor volverá a validar cada saldo antes de guardar." size="xl"><div className="mb-4 grid gap-3 sm:grid-cols-2"><Input controlKey="payments.manual.period" permission="ApplyPayments" type="month" label="Mes y año de la mensualidad" value={period} onChange={event => setPeriod(event.target.value)} /><Select controlKey="payments.manual.subscription" permission="ApplyPayments" label="Suscripción del plan" value={subscription || "all"} onValueChange={value => setSubscription(value === "all" ? "" : value)}><SelectItem value="all">Todas las suscripciones</SelectItem>{subscriptions.map(code => <SelectItem key={code} value={code}>{code}</SelectItem>)}</Select></div><p className="mb-3 text-sm text-muted">Puedes combinar meses distintos. El filtro solo cambia las opciones disponibles; conserva las mensualidades ya seleccionadas.</p><div className="space-y-3">{lines.map((line, index) => <div key={index} className="grid gap-3 rounded-card border border-border bg-surface-subtle p-3 lg:grid-cols-[1fr_1fr_150px_44px] lg:items-end"><Combobox controlKey="payments.manual.payment" permission="ApplyPayments" label="Pago" value={line.ledgerEntryId} onValueChange={(value) => update(index, "ledgerEntryId", value)} options={paymentOptions} placeholder="Seleccionar pago…" /><Combobox controlKey="payments.manual.charge" permission="ApplyPayments" label="Mensualidad / suscripción" value={line.billingItemId} onValueChange={(value) => update(index, "billingItemId", value)} options={chargeOptions} placeholder="Buscar mensualidad…" /><Input controlKey="payments.manual.amount" permission="ApplyPayments" label="Importe" type="number" min="0.01" step="0.01" value={line.amount} onChange={(event) => update(index, "amount", event.target.value)} /><Button controlKey="payments.manual.line.remove" permission="ApplyPayments" variant="ghost" size="icon" aria-label="Eliminar línea" disabled={lines.length === 1} onClick={() => setLines((items) => items.filter((_, position) => position !== index))}><X className="h-4 w-4" /></Button></div>)}</div><Button controlKey="payments.manual.line.add" permission="ApplyPayments" variant="ghost" className="mt-3" onClick={() => setLines((items) => [...items, { ledgerEntryId: "", billingItemId: "", amount: "" }])}><Plus className="h-4 w-4" />Agregar otra mensualidad</Button><DialogFooter><Button controlKey="payments.manual.cancel" systemRequired variant="secondary" onClick={() => close(false)}>Cancelar</Button><Button controlKey="payments.manual.submit" permission="ApplyPayments" loading={apply.isPending} disabled={!valid} onClick={async () => { const allocations: PaymentAllocationLineRequest[] = lines.map((line) => ({ ledgerEntryId: line.ledgerEntryId, billingItemId: line.billingItemId, amount: Number(line.amount) })); await apply.mutateAsync({ allocations, idempotencyKey: crypto.randomUUID() }); close(false); }}><CheckCircle2 className="h-4 w-4" />Confirmar aplicación</Button></DialogFooter></DialogContent></Dialog>;
}
